...
Do NOT modify the existing `createDryingLog` function -- keep it as-is for backward compatibility. The new `createDryingLogWithRooms` is an alternative that adds room pre-population.
  ...
  <verify>
**Code-based verification (primary):**

1. Run `node -e "const m = require('./src/db/dryingLogs.js'); console.log(typeof m.createDryingLogWithRooms)"` from the `backend/` directory -- must print `function`, confirming the new transaction is exported
2. Grep `backend/src/routes/drying.js` for `areas_affected` -- must appear in the POST /log handler, confirming the route reads and parses the job's areas_affected field
3. Grep `backend/src/routes/drying.js` for `createDryingLogWithRooms` -- must appear, confirming the route calls the new transaction instead of the old createDryingLog
4. Read the POST /log handler and verify the response structure includes `{ log, chambers, rooms }` (not just `{ log }`)
5. Read `createDryingLogWithRooms` in dryingLogs.js and verify it: (a) creates a log, (b) creates a default chamber, (c) iterates roomNames to create rooms, (d) returns the composite object

**Manual/optional (requires running server):**

- `curl -X POST http://localhost:3000/api/apex-jobs/{JOBID}/drying/log -H "Cookie: ..."` -- should return 201 with `{ log, chambers, rooms }` where rooms match the job's areas_affected parsed values
- Calling POST again should return 409 with the existing log
- For a job with empty areas_affected, POST should return 201 with `{ log, chambers: [{name: 'Default', ...}], rooms: [] }`
  </verify>
  <done>
...
1. **`_loadDryingState(jobId)`** -- Async method that:
   ...
   - On 404 error (check `err.message.includes('404') || err.message.includes('No drying log')` -- the `api.request()` method throws `new Error(data.error || data.message || \`HTTP ${response.status}\`)` so the error message will be either the backend text `"No drying log for this job"` or the fallback `"HTTP 404"`. Error objects do NOT have a `.status` property): checks activeTab again, then sets `container.innerHTML` to `_renderCreateDryingButton(jobId)`
